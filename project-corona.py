...
def handle_dialog(req, res):
    ...
    try:
        if list(map(lambda x: x.lower(), req['request']['nlu']['tokens'])) == ['закончить', 'диалог'] or list(map(lambda x: x.lower(), req['request']['nlu']['tokens'])) == ['завершить', 'диалог']:
    ...
    if parts[user_id] == 0:
        ...
        if 'нет' in list(map(lambda x: x.lower(), req['request']['nlu']['tokens'])) or 'не хочу' in list(map(lambda x: x.lower(), req['request']['nlu']['tokens'])):
            ...
            res['response']['tts'] = 'Ты не определился, скажи да sil <[300]> или нет'
